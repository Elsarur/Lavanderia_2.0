...
@order_bp.route("/get_order_detail/<int:order_id>", methods=["GET"])
def get_order_detail_endpoint(order_id):
    try:
        order = get_order_detail(order_id)
        return jsonify({"msg":"Detalle de orden obtenido", "data": order}), 200
    except Exception as e:
        return jsonify({"msg": "Ociurrio un error", "error":e}), 500